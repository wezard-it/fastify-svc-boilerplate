...
resource "google_artifact_registry_repository" "my-repo" {
  ...
  description   = "Staging docker repository"
  format        = "DOCKER"
...
resource "local_file" "env_file" {
  filename = "${path.module}/../../.env.staging"
  content  = file("${path.module}/../../.env.staging")
}
...
resource "google_cloud_run_v2_service" "cloud_run_service" {
  ...
  template {
    containers {
        ...
        env {
          name = "DATABASE_URL"
          value = "postgresql://${google_sql_user.db_user.name}:${var.db_pwd}@${google_sql_database_instance.postgres_instance.public_ip_address}:5432/${google_sql_database.database.name}"
        }
        env {
